...
export const duplicateCaseSchema = z.object({
  ...
  sourceEventId: localIdSchema.optional(),
...
export const sbcProposalSchema = z.object({
  ...
  requiredPlayers: z.number().int().min(1).max(23),
  requiredRating: z.number().int().min(1).max(99),
  strategy: z.enum(['duplicate-cleanup', 'club-preservation', 'low-coin-cost']),
